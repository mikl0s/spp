...
echo "$out" | grep -q "claude.*/.claude/skills  ($n_skills/$n_skills skills)" \
  && pass "check reports $n_skills/$n_skills claude skills" \
  || fail "check did not report $n_skills/$n_skills claude skills"
